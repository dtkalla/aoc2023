arr = ["2911threeninesdvxvheightwobm
...
arr.each do |str|
    temp = []
    ["dtkalla","one","two","three","four","five","six","seven","eight","nine"].each_with_index do |num,i|
        if str.index(num)
            idx = str.index(num)
            temp << [idx,i.to_s]
            indx = str.rindex(num)
    ...
    str.each_char.with_index do |char,i|
        temp << [i,char] if ("0".."9").include?(char)
    end
    p temp
    temp.sort!
    total += (temp[0][1] + temp[-1][1]).to_i
end
