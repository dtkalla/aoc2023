...
s = ".242......276....234............682.......................958..695..742................714......574..............833.........159....297.686.
.............*............................612*......304..*..........*.......@175...#...*...........*890...........*.............*..*........
...
def gears(i,j,k,num)
    (i-1..i+1).each do |x|
        (j-1..k).each do |y|
            if (0...@m).cover?(x) && (0...@n).cover?(y) && @grid[x][y] == '*'
                @gear[[x,y]] << num
            end
        end
    end
...
(0...@m).each do |i|
    (0...@n).each do |j|
        if @nums.include?(@grid[i][j]) && !set.include?([i,j])
            str = []
            k = j
            while @grid[i][k] && @nums.include?(@grid[i][k])
                set.add([i,k])
                str << @grid[i][k]
                k += 1
            end
...
total = 0
@gear.values.each { |arr| total += arr[0] * arr[1] if arr.length == 2 }
p total
