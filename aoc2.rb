...
games = s.split("Game").map { |str| str.split(":")[1] }
games.shift
total = 0
games.each do |game,i|
    reveals = game.split(";").map { |subarr| subarr.split(',') }.flatten
    red, green, blue = 0, 0, 0
    reveals.each do |reveal|
        if reveal.index("red")
            red = [red, reveal.to_i].max
        elsif reveal.index("green")
            green = [green, reveal.to_i].max
        elsif reveal.index("blue")
            blue = [blue, reveal.to_i].max
        end
    end
