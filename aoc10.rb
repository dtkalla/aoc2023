...
grid = new_grid
grid.pop
grid.map! { |row| row.join('.').split('') }

m,n = grid.length,grid[0].length

(0...m).each do |i|
    queue = []
    queue << [i,0] unless checked.include?([i,0])
    queue << [i,n-1] unless checked.include?([i,n-1])
    checked.add([i,0])
    checked.add([i,n-1])
    until queue.empty?
        i,j = queue.shift
...
(1...n-1).each do |i|
    queue = []
    queue << [0,i] unless checked.include?([0,i])
    queue << [m-1,i] unless checked.include?([m-1,i])
    checked.add([0,i])
    checked.add([m-1,i])
    until queue.empty?
        i,j = queue.shift
        dirs.each do |dir|
            a,b = dir[0..1]
            c,d = a+i,b+j
            if !checked.include?([c,d]) && (0...m).cover?(c) && (0...n).cover?(d)
                checked.add([c,d])
                queue << [c,d]
            end
        end
...
(0...m).each do |i|
    (0...n).each do |j|
        total += 1 if (!checked.include?([i,j]) && i % 2 == 0 && j % 2 == 0)
    end
end
